...
- **Concern A (from the final whole-branch review) — RESOLVED, live-confirmed rather than left as an open question.** The precedence question this row originally asked ("does the OSS shim actually get reached on a machine with a pre-existing managed-settings.json from ordinary Claude usage") was answered empirically on the very first real `ug claude` run: it does not. `managed-settings.json`'s `env.ANTHROPIC_BASE_URL` (pointing at the real `/ai-gateway/anthropic` route) won over both the per-launch `--settings` file's loopback URL and the loopback URL already present in the subprocess env — Claude Code sent `databricks-kimi-k3` straight to the real gateway, which 400s (`API type 'anthropic/v1/messages' is not supported by 'databricks-kimi-k3'`). A passive warning would not have prevented this. Fixed in commit `677860e`: `_reconcile_managed_settings`'s OSS-shim branch now calls `_managed_relayed_conflicts` (already used by the relayed path for the identical risk) and raises before Claude Code ever launches, naming the conflicting keys and pointing at `ucode revert` — mirroring how relayed already handles this. A managed `ANTHROPIC_BASE_URL` that's already a loopback URL is not flagged (it's a harmless leftover from an earlier relayed/OSS-shim session, not a genuine conflict). Re-verify during Task 9 that a machine with a genuinely conflicting managed file now fails at `ug claude` configure time with a clear message, rather than launching into a broken session.
