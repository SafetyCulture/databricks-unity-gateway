...
- **Added after the final whole-branch review's fix wave, unresolved question it surfaced (Concern A):** on a machine that has ALREADY run a normal, interactive `ug claude`/`ug configure` against a workspace that DOES have real Claude models (so `~/Library/Application Support/ClaudeCode/managed-settings.json` or `/etc/claude-code/managed-settings.json` already has `env.ANTHROPIC_BASE_URL`/`apiKeyHelper`/Claude model pins from that session), switch to an OSS-fallback workspace and launch `ug claude`. Confirm the session actually reaches the local shim (the trace log via `CLAUDE_OSS_SHIM_LOG=/tmp/trace.jsonl` shows a request, and `/model` lists GLM/Kimi, not stale Claude ids) rather than silently connecting to the OLD workspace's real Claude endpoint using the stale managed-file credentials. Record the answer here. If it does NOT reach the shim (managed settings win over the per-launch `--settings` file for these keys), the OSS-shim launch path (`claude.py`'s managed-mirror skip added in the final fix wave) needs a warning — reuse the existing read-only `_managed_relayed_conflicts(path)` helper to detect a conflicting `apiKeyHelper`/`env.ANTHROPIC_BASE_URL`/`env.ANTHROPIC_CUSTOM_HEADERS` in the managed file on the OSS path and `print_warning` naming the keys + pointing at `ucode revert`, without hard-failing the launch. If it DOES reach the shim correctly, this concern is closed with no code change needed.
